...
The package knows no harness concepts and serves no files: the `/api` HTTP bridge and downlink WebSockets are routes owned by the connection plugin, plugin bundles and the HMR event stream are routes owned by the modules/hmr plugins, and dist serving belongs to the fallback owner. The upgrade handler owns the protocol handshake and connection contents; the webserver only delivers the raw socket and request. `host` accepts only `127.0.0.1` (default posture) and `0.0.0.0` (deliberate network exposure). The shipped Electron shell reuses this server rather than replacing it: it starts the same host as a child process and points its window at the loopback URL, so the browser and desktop clients share one carrier. This package never prints; the URL line belongs to the shell.
